  ...
  return (
    ...
    >
      ...
      <Link href={`/product/${product.slug}`} className="relative block aspect-square bg-[#F8F5F0] img-zoom" aria-label={`View ${product.name}`}>
        ...
        {discount && (
          <span className="absolute top-3 right-3 text-[11px] font-bold px-2 py-1 rounded-full bg-red-600 text-white">
            -{discount}%
          </span>
        )}
        <button
      ...
      <div className="p-4 flex flex-col flex-1">
        ...
        <div className="flex items-center justify-between gap-2 mt-3">
          <div>
            <span className="text-lg font-black text-[#111]" style={{ fontVariantNumeric: 'tabular-nums' }}>
              ₪{product.price.toLocaleString()}
            </span>
            {product.originalPrice && (
              <span className="ml-1.5 text-sm text-[#888] line-through">₪{product.originalPrice.toLocaleString()}</span>
            )}
          </div>
          <button
            onClick={handleAdd}
